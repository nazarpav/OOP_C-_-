...
Queue::Queue::Queue(const Queue & other):Queue(other.maxSize)
{
	for (int i = 0; i < 2; i++)
	{
...
void Queue::Queue::Add(int elem,int priority)
{
...
bool Queue::Queue::IsFull() const
{
	// Максимально заповнена?
	return size == maxSize;
}

int Queue::Queue::GetCount() const
{
	// Розмір черги
	return size;
}

void Queue::Queue::Clear()
{
	// Очистка черги
	size = 0;
}

void Queue::Queue::Show() const
{
	std::cout << "\n-----------------------------------\n";
	// Показ черги
	for (int i = 0; i < size; i++)
