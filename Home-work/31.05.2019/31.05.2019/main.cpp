...
void show_players(Player *&Players, const unsigned short &size)
{
	system("cls");
...
unsigned short search_data_in_arr(Player *&player_arr, const unsigned short &size)
{
	system("cls");
...
void SaveInFileBinary(Player *& player, unsigned short &size)
{
	...
	out.write((char*)&size, sizeof(player));
	for (unsigned short i = 0; i < size; i++)
	{
		out.write((char*)&player[i], sizeof(player[i]));
	}

	out.close();
}

void LoadFromFileBinary(Player *& player_arr, unsigned short &size)
{
	ifstream in("playerBin.txt", ios_base::in | ios_base::binary);

	in.read((char*)&size, sizeof(Player));
	for (unsigned short i = 0; i < size; i++)
	{
		in.read((char*)&player_arr[i].name, sizeof(Player));
		in.read((char*)&player_arr[i].surname, sizeof(Player));
		in.read((char*)&player_arr[i].age, sizeof(Player));
		in.read((char*)&player_arr[i].games, sizeof(Player));
		in.read((char*)&player_arr[i].points, sizeof(Player));
		in.read((char*)&player_arr[i].avgPointPerGame, sizeof(Player));
	}
in.close(); 
}
...
int main()
{
	...
	const unsigned short size_menu = 9; // size menu
	...
	while (flag == true)
	{
		...
		case 6:
			...
			break;
		case 7:
			LoadFromFileBinary(player_arr, size);
			break;
		case 8:
			SaveInFileBinary(player_arr, size);
			break;
		case 9:
			flag = false;
			break;
